...
4.  **Feature Selection (Poda Arquitectónica):** Tras la extracción de valor (como la edad y el cruce espacial), se descartaron múltiples variables crudas de alta cardinalidad (nombres de hospitales, fechas, IDs de distritos) para evitar colapsos de memoria durante el Machine Learning (*MemoryError*). Sin embargo, se tomó la decisión experimental de **conservar temporalmente las variables `Hemoglobina` y `Dx_anemia`** para evaluar el comportamiento algorítmico frente a predictores pos-diagnóstico (prueba de estrés).
